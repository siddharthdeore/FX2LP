...
int main()
{
    ...
    f = fopen("ep2_fw.ihx", "r"); // load firmware from intel hex file
    if (f == NULL)
    {
        printf("\n\nCould not find file: ep2_fw.ihx\n\n");
        exit(0);
    }
    while (!feof(f))
    {
    ...
    while (keepRunning)
    {
        //for (j = 0; j < 5; j++)
        {
            ...
            //usleep(10);
            int received_count = usb_bulk_read(current_handle, endpoint, reply, 64, 100); // receive reply
            usleep(100);
            
            gettimeofday(&tv, NULL);
            double time_in_sec = tv.tv_sec + (double)tv.tv_usec/1e6;
            for (i = 0; i < 64; i++)
            {
                // printf(" %02x", reply[i]);
                printf("\n %lf " BYTE_TO_BINARY_PATTERN, time_in_sec,BYTE_TO_BINARY(reply[i]));
            }
