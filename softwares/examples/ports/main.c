//ports.c   a test of port operation
//cypress FX2   68013
//
...
//compile:  gcc -lusb ports.c -o ports
...
f=fopen("ports_fw.ihx", "r");		//load firmware from intel hex file
if (f==NULL)
	{printf("\n\nCould not find file: ports_fw.ihx\n\n");exit(0);}
while(!feof(f)){
...
buffer[0]=0x1;buffer[1]=0;buffer[2]=0;buffer[3]=0;buffer[4]=0;buffer[5]=0x55;
er[9]=usb_bulk_write(current_handle, endpoint, buffer, 6, 1000);
sleep(0.1);
er[10]=usb_bulk_read(current_handle, endpoint, buffer, 5, 1000);
sleep(0.1);
printf("\n Port directions: ");for (i=0;i<5;i++) printf(" %02x", buffer[i]); 
printf("    Status: %d  %d \n",er[9],er[10]);
...
for (j=0;j<5;j++)
	...
	er[9]=usb_bulk_write(current_handle, endpoint, buffer, 1, 1000);  //send command
	sleep(0.1);
	er[10]=usb_bulk_read(current_handle, endpoint, buffer, 5, 1000);  //receive reply
	sleep(0.1);
	printf("\n Ports: ");for (i=0;i<5;i++) printf(" %02x", buffer[i]); 
	printf("    Status: %d  %d \n",er[9],er[10]);
	}

//next we set ports B and D to outputs

buffer[0]=0x1;		//"set port directions" command
buffer[1]=0;buffer[2]=0xFF;buffer[3]=0;buffer[4]=0xFF;buffer[5]=0;  //ports B,D outputs 
er[9]=usb_bulk_write(current_handle, endpoint, buffer, 6, 1000);    //send command
sleep(0.1);
er[10]=usb_bulk_read(current_handle, endpoint, buffer, 5, 1000);    //receive to verify
sleep(0.1);
printf("\n Smeri: ");for (i=0;i<5;i++) printf(" %02x", buffer[i]); 
printf("    Status: %d  %d \n",er[9],er[10]);

//and toggle all of the ports B and D bits (like blinking a LED)

printf("\n\n Watch ports B and D bits changing!!\n\n");

buffer[0]=0x3;		//"write all ports" command
for (j=0;j<10;j++)
	{
	buffer[2]=0xFF;buffer[4]=0xFF;
	er[9]=usb_bulk_write(current_handle, endpoint, buffer, 6, 1000);   //set to all ones
	sleep(1);
	buffer[2]=0;buffer[4]=0;
	er[10]=usb_bulk_write(current_handle, endpoint, buffer, 6, 1000);  //set to all zeros
	sleep(1);
	printf("    Status: %d  %d \n",er[9],er[10]);
	}

//last we read the ports again

buffer[0]=0x2;
er[9]=usb_bulk_write(current_handle, endpoint, buffer, 1, 1000);
sleep(0.1);
er[10]=usb_bulk_read(current_handle, endpoint, buffer, 5, 1000);
sleep(0.1);
printf("\n Ports: ");for (i=0;i<5;i++) printf(" %02x", buffer[i]); 
